...
Per determinare la potenza erogata da un generatore, si calcola la tensione, se generatore Northon, o la corrente, se generatore Thevenin, di lato, nella convenzione dei 
...
invece, bisogna considerare il contributo di ogni lato. Quindi solo nella risoluzione con la matrice delle conduttanze modali, e delle resistenze modali per il metodo 
...
\begin{gather*}
    [I_{mo}]:=(i_{{mo}_{i}})\\
    i_{{mo}_i}=\displaystyle\sum_{k=1}^{n_i}\pm i_{g_k}
\end{gather*}
...
\begin{gather*}
    ...
    \end{bmatrix}
\end{gather*}
Si esrpimono le equazioni banali o di vincolo, che esprimono la relazione delle potenze modali con le tensioni erogate dalle forme Thevenin estreme: 
\begin{gather*}
    V_A=-4\,V\\
    V_B-V_C=8\,V\\
    \begin{bmatrix}
...
forme Northon equivalenti e la somma delle conduttanze di lato:
\begin{gather*}
...
Si esprimono ora le relazioni del metodo dei nodi su questo nuovo circuito, considerando la forma Northon corrispondente del lato Thevenin presente: 
\begin{gather*}
    \begin{bmatrix}
...
dei coefficienti $[coef]$ e si riportano oltre al vettore dei termini noti di partenza $[I_{mo}]$ o $[E_m]$, in base al metodo usato, le grandezze di vincolo note, tolte dal 
vettore delle incognite, $[X_{no}]$. 
\begin{gather*}
    [coef][X]=[I_{mo}/E_m]+[X_{no}]
\end{gather*}
Si definisce così un nuovo vettore dei termini noti $[T_{no}]$, per risolvere il sistema rispetto alle nuove grandezze:
\begin{gather*}
    [T_{no}]=[I_{mo}/E_m]+[X_{no}]\\
    [X]=[coef]^{-1}[T_{no}]
\end{gather*}
