...
\begin{gather*}
    \mathscr{L}\{Af(t)+Bg(t)\}=\displaystyle\int_{0^-}^{+\infty}\left(Af(t)+Bg(t)\right)dt=A\int_{0^-}^{+\infty}f(t)dt+B\int_{0^-}^{+\infty}g(t)dt=aF(s)+bG(s)
\end{gather*}
...
L'argomento della funzione esponenziale deve essere adimensionale, per cui la variabile complessa $s$ ha una dimensione tale da rendere l'argomento adimensionale:
\begin{gather*}
...
\end{gather*}
Data la trasformata della tensione, si sostituisce $j\omega$:
\begin{gather*}
