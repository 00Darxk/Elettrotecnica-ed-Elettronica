...
Si definisce la trasformata di Laplace $F(s)$ della funzione $f(x)$ un funzionale $\mathscr{L}\{f(x)\}=F(s)$ che trasferisce una funzione dal dominio del tempo al domino 
...
\begin{equation}
    \mathscr{L}\{u_{-1}(t)\}=\displaystyle\frac{1}{s}
\end{equation}
...
\begin{gather}
    \mathscr{L}\{e^{-\alpha t}\}=\displaystyle\frac{1}{s-\alpha}
\end{gather}
...
\begin{gather*}
    \mathscr{L}\{f(t)\}=F(s)\\
    \mathscr{L}\left\{\displaystyle\frac{d}{dt}f(t)\right\}=G(s)\\
...
\begin{equation}
    \mathscr{L}\left\{\displaystyle\frac{df(t)}{dt}\right\}=s\mathscr{L}\{f(t)\}-f(0^-)
\end{equation}
Tramite questa definizione è possibile calcoalre ricorsivamente la trasformata di una derivata $n-$esima di una funzione $f$ di trasformata nota:
\begin{gather*}
    \mathscr{L}\left\{\displaystyle\frac{d}{dt}\frac{df}{dt}\right\}=s\left[sF(s)-f(0^-)\right]-\dot f(0^-)=s^2F(s)-sf(0^-)-\dot f(0^-)\\
    \mathscr{L}\left\{\displaystyle\frac{d^nf}{dt^n}\right\}=s^nF(s)-\sum_{k=0}^{n-1}s^{n-1-k}\frac{d^kf}{dt^k}\Bigg|_{t=0^-}
\end{gather*}

La trasformata di una combinazione lineare è una combinazione lineare delle trasformate:
\begin{gather*}
    \mathscr{L}\{Af(t)+Bg(t)\}=\displaystyle\int_{0^-}^{+\infty}\left(Af(t)+Bg(t)\right)dt\\
    A\int_{0^-}^{+\infty}f(t)dt+B\int_{0^-}^{+\infty}g(t)dt=AF(s)+BG(s)
\end{gather*}
Quindi vale la seguente proprietà:
\begin{equation}
    \mathscr{L}\{A(f)+Bg(t)\}=A\mathscr{L}\{f(t)\}+B\mathscr{L}\{g(t)\}
\end{equation}

Si considera la proprietà della derivazione considerando una funzione integrale $f(t)$:
\begin{gather*}
    f(t)=\displaystyle\int_{0^-}^tg(\tau)d\tau\\
    \mathscr{L}\{f(t)\}=F(s)=\mathscr{L}\left\{\displaystyle\int_{0^-}^tg(\tau)d\tau\right\}\\
    \mathscr{L}\left\{\displaystyle\frac{df(t)}{dt}\right\}=sF(s)=G(s)=\mathscr{L}\{g(t)\}
\end{gather*}
A memoria nulla quindi $g(0^-)=0$, la trasformata di un integrale si ottiene come:
\begin{equation}
    \mathscr{L}\left\{\displaystyle\int_{0^-}^tg(\tau)d\tau\right\}=\frac{\mathscr{L}\{g(t)\}}{s}
\end{equation}


Si applicano queste prorpietà nell'analisi di un circuito RLC serie:
\begin{center}
    \begin{circuitikz}
...
Si vuole determinare la dimensione delle grandezze nel dominio di Laplace, per cui si conisderano i componenti singolarmente. Dato un solo induttore nell convenzione degli 
...
\end{gather*}
Per cui si può considerare il componente $z(s)$ un'impedenza, poiché mantiente la sua grandezza nel trasferimento dal tempo continuo al dominio di Laplace. Mentre la tensione 
...
\begin{gather*}
    ...
    R_3=6\,\Omega
\end{gather*}
La tensione ai capi del condesnatore si esprime considerando un generatore di tensione per la memoria:
\begin{gather}
    C=0.05\,F\\
...
Si considera un circuito RLC serie a regime permanente, e si vuole determinare la sua espressione a regime nel dominio di Laplace. Precedentemente è stato analizzato nel domino 
dei fasori:
\begin{gather*}
...
\begin{gather*}
    \mathscr{L}\{V_M\sin\omega t\}=\mathscr{L}\left\{\displaystyle\frac{V_M}{2j}\left(e^{j\omega t}-e^{-j\omega t}\right)\right\}\\
...
\begin{gather*}
    \mathscr{L}\{A\cos(\omega t)\}=A\displaystyle\frac{e^{j\omega t}+e^{-j\omega t}}{2}=\frac{A}{2}\frac{s+j\omega+s-j\omega}{s^2+\omega^2}=\frac{As}{s^2+\omega^2}
\end{gather*}

Si considera ora una sinusoide sfasata di un fattore $\varphi$ e si vuole determinare la sua trasformata:
\begin{gather*}
    \mathscr{L}\{A\sin(\omega t+\varphi)\}=\mathscr{L}\{A\cos\varphi\sin(\omega t)+A\sin\varphi\cos(\omega t)\}=\displaystyle A\cos\varphi\frac{\omega}{s^2+\omega^2}+A\sin\varphi\frac{s}{s^2+\omega^2}\\
    \mathscr{L}\{A\sin(\omega t+\varphi)\}=\displaystyle\frac{A\cos\varphi \omega+A\sin\varphi s}{s^2+\omega^2}
\end{gather*}


Si considera una funzione esponenziale moltiplicata per un fattore lineare:
\begin{gather*}
    \mathscr{L}\{Ate^{\alpha t}\}=F(s)\\
    \mathscr{L}\left\{\displaystyle\frac{d}{dt}\left(Ate^{\alpha t}\right)\right\}=\mathscr{L}\{Ae^{\alpha t}+A\alpha te^{\alpha t}\}=A\mathscr{L}\{e^{\alpha t}\}+\alpha\mathscr{L}\{Ate^{\alpha t}\}=sF(s)-f(0^-)\\
    \displaystyle\frac{A}{s-\alpha}+\alpha F(s)=sF(s)\\
    \mathscr{L}\{Ate^{\alpha t}\}=\frac{A}{(s-\alpha)^2}
\end{gather*}
...
\end{gather*}
Per cui è leggittimo definire l'impedenza a regime come rapporto tra queste due grandezze, calcolato al valore dei poli. Questa relazione persiste anche tra due grandezze 
...
possible solo se $j\omega$ non rappresenta un polo nel dominio di Laplace. Dove un polo $s_p$ è definito come un valore per cui la trasformata tende a valori infiniti:
\begin{gather*}
    \lim_{s\to s_p}F(s)=\pm\infty
\end{gather*}
Nella trasformata di una funzione sinusdoiale i poli sono esattamente pari a $\pm j\omega$, per cui non è possibile determinare l'espressione di una grandezza nel domino 
dei fasori, data la sua trasformata di Laplace, se nel tempo continuo la sua espressione a regime è una sinusoide. 

\subsection{Bipoli in Laplace}

Tra i due domini non si mantiene lo stesso simbolo nella loro rappresentazione circuitale. Per un resistore la legge costitutiva tra i due domini diventa:
\begin{gather*}
    v(t)=Ri(t)\to V(s)=RI(s)
\end{gather*}
\begin{center}
...
\end{gather*}
Per cui nel dominio di Laplce corrisponde ad un lato Thevenin/Norton:
\begin{center}
    \begin{circuitikz}
        \draw (0,0) to[short, i=$i(t)$,*-](0.5,0)
                    to[L=$L$,-*](2.5,0);
...
\begin{center}
    \begin{circuitikz}
        ...
        \draw (3,0) to[short,i=$I(s)$,*-](3.5,0)
                    to[generic=$\displaystyle\frac{1}{sC}$,-*] (5.5,0)
                    to[european voltage source=$\displaystyle\frac{v(0^-)}{s}$](7.5,0);
        \draw[->] (5.25,-0.5)--(3.25,-0.5)node[midway, below]{$V(s)$};
        ...
        \draw(10.5,1) to[short](10.5,0)
                    to[short,-*](11,0);
        \draw(8.5,0)to[short](8.5,-1)
                    to[generic=$\displaystyle\frac{1}{sC}$](10.5,-1)
                    to[short](10.5,0);
        \draw[->] (10.75,-1.5)--(8.25,-1.5)node[midway, below]{$V(s)$};
...
\begin{center}
    \begin{circuitikz}
        \draw (0,0) to[european voltage source=$\displaystyle\frac{E}{s}$](0,4)
                    to[generic=$R$,-*](2,4)node[above]{$A$};
        \draw (0,0) to[short,-*](2,0)node[below left]{$B$}
                    to[european voltage source=$\displaystyle\frac{v_C(0^-)}{s}$](2,2)
                    to[generic=$\displaystyle\frac{1}{sC}$](4,2);
...
\end{gather*}
Le frazioni si dicono proprie se il grado del denominatore è maggiore del grado del numeratore $m>n$, altrimenti se il grado del numeratore è maggiore si dicono impriorie $n>m$. 
In caso siano improrie bisogna prima effettuare una divisione tra polinomi, ritornando una frazione propria ed polinomio.
...
\end{gather*}
Si individua un primo polo $s_p=-\frac{1}{RC}$, al finito. Con molteplicità uno. Coincide alla soluzione dell'equazione caratteristica associata all'equazione differnziale 
