...
conduttanze ed i lati Northon ai lati Thevenin, nelle formule risolutive. 
...
\begin{gather*}
    R_{m_{ij}}:=\begin{cases}
        \displaystyle\sum_{k=1}^{n_i}R_{k_i}&\forall i=j\\
        \displaystyle-\left(\sum_{k=1}^{n_{ij}}R_{k_{ij}}\right)&\forall i\neq j
    \end{cases}\\
    [R_m]=\begin{bmatrix}
...
\subsubsection{Esempio Applicativo di Una Rete Pilotata}

Risolvendo reti dove sono presenti generatori controllati, in prima analisi vengono analizzati come fossero dei generatori indipendenti. 
...
%% 6/11: riscrivere, con esempi applicativi
...
incognite. Per poter risolvere il sistema, si considerano equazioni banali o di vincolo, per poter diminuire il numero delle incognite totali del sistema. Si sostituiscono 
nel sistema, e si decide un nuovo vettore delle incognite $[X]$, se corrispondono a grandezze diverse, si dice di aver ibridizzato il sistema. Dopo aver definito questo nuovo 
vettore, bisogna esprimere la matrice dei coefficienti $[coeff]$ ed il nuovo vettore termini noti $[Y]$ adeguatamente per mantenere le uguaglianze precedenti:
\begin{gather*}
    [coeff][X]=[Y]\\
    [X]=[coeff]^{-1}[Y]
\end{gather*}
...
\end{gather*}
Per verirficare questa soluzione, si può risolvere il circuito considerando un resistore di resistenza $R_5=5\,\Omega$ tra i due morsetti, rispetto alla tensione modale 
...
Come gi à noto, la forma Thevevnin è la duale della forma Norhton, ed è possibile passare tra le due forme. Per cui è possibile, nota la rappresentazione Thevenin esterna di 
...
Thevnin e Northon, secondo le formule di passaggio già note:
\begin{gather*}
    R_{th}=R_{No}\\
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \draw[dashed](1,1.5)--(1,-3)node[below]{$\tau$};
    \end{tikzpicture}
\end{center}
...
\end{gather*}
Poiché $\sigma<0$, l'ampiezza dell'oscillazione della sinusoide diminuisce nel tempo, quindi è u ìn oscillatorio smorzato. Questo caso si chiama quindi sottosmorzato. 
...
rappresentato dalla curva di colore rosso, ed il caso critico, rappresentato dalla curva di colore blu. Si considera un unico andamento per la corrente 
nel caso sovrasmorzato e nel caso limite, poiché sono due funzioni della stessa classe di equivalenza, entrambe combinazione lineare di esponenziali reali. 
\begin{align*}
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        \draw[->](-0.5,0)--(3,0)node[above]{$t$};
        \draw[->](0,-0.5)--(0,1.5)node[right]{$v_C(t)$};
